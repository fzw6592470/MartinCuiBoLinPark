...
let vlogin = {
	  ...
	  inputPassword: function(e){
	  		...
	  		if(this.data.repassword !="" && password !== this.data.repassword){
	  		...
	  		let varifyCode = this.data.varifyCode;
	  		if(/^1[3,5,7,8]{1}[0-9]{9}$/g.test(userName) && /^\d{6}$/.test(varifyCode)){
	  				this.setData({
					  		allow: false
			  		})
	  		}
	  ...
	  inputRePassword: function(e){
	  		...
	  		if(repassword.length < 8 || !/[0-9]+[a-zA-Z]+[0-9a-zA-Z]*|[a-zA-Z]+[0-9]+[0-9a-zA-Z]*/.test(repassword)){
	  				this.showToast("请输入8位以上的数字与字母的组合");
	  				this.setData({
					  		allow: true
			  		})
	  		...
	  		if(this.data.repassword !="" && repassword !== this.data.password){
	  				this.showToast("二次输入的密码不一致");
	  				this.setData({
					  		allow: true
			  		})
